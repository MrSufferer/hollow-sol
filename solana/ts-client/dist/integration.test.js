...
// ============================================================================
import { address, createKeyPairSignerFromBytes, createSolanaRpc, createSolanaRpcSubscriptions, createTransactionMessage, appendTransactionMessageInstructions, setTransactionMessageFeePayerSigner, setTransactionMessageLifetimeUsingBlockhash, signTransactionMessageWithSigners, assertIsSendableTransaction, assertIsTransactionWithBlockhashLifetime, sendAndConfirmTransactionFactory, lamports, getProgramDerivedAddress, getAddressEncoder, } from "@solana/kit";
import { getSetComputeUnitLimitInstruction } from "@solana-program/compute-budget";
import { getTransferSolInstruction, SYSTEM_PROGRAM_ADDRESS, } from "@solana-program/system";
import fs from "fs";
import path from "path";
// Re-export Fr from merkle-tree module since it successfully imports it
// We'll import the functions we need from merkle-tree
import { createPoseidonTree } from "./merkle-tree.js";
// Since merkle-tree.ts imports Fr successfully, let's try importing it the same way
// But first, let's check if we can just use the tree's internal functions
// Actually, let's create a helper that uses the tree's hashLeftRight pattern
async function getFrFromTree() {
    // Create a temporary tree to access Fr
    const tree = createPoseidonTree(1);
    // Insert a dummy value to trigger hashLeftRight which uses Fr
    // Actually, better: let's just import Fr the same way merkle-tree does
...
const VERIFIER_PROGRAM_ID = address("D9YEdaR4MT37wUP1GZ1CmrirMtRsegG7gibX3rLb7sgD");
// Mixer program ID - deployed to devnet
const MIXER_PROGRAM_ID = address(process.env.MIXER_PROGRAM_ID || "267wWpf21MBLmyHD9j7ausHG6FR8gjMS3758zsMTTjBT");
...
async function getBalance(ctx, address) {
    const result = await ctx.rpc.getBalance(address).send();
    return result.value;
}
// ============================================================================
// Merkle Tree Helpers (using Poseidon2)
// ============================================================================
const ZERO_VALUES = [
...
async function hashLeftRight(left, right) {
    ...
    const Fr = await getFrClass(bb);
    const frLeft = Fr.fromString(left);
    const frRight = Fr.fromString(right);
    // Use the method that exists on this Barretenberg instance
    const hashMethod = bb.poseidon2Hash || bb.poseidonHash;
    if (!hashMethod) {
        throw new Error("No poseidon hash method found on Barretenberg");
    }
    const hash = await hashMethod.call(bb, [frLeft, frRight]);
...
// Proof Generation
// ============================================================================
function writeProverToml(inputs) {
...
async function main() {
    ...
    const payer = await loadKeypair(path.join(process.cwd(), "../keypair/deployer.json")).catch(() => {
    ...
    if (payerBalance < lamports(2000000000n)) {
    ...
    const initMessageWithPayer = setTransactionMessageFeePayerSigner(payer, initMessage);
    const initMessageWithLifetime = setTransactionMessageLifetimeUsingBlockhash(latestBlockhash, initMessageWithPayer);
    const initTransactionMessage = appendTransactionMessageInstructions([getSetComputeUnitLimitInstruction({ units: 200000 }), initIx], initMessageWithLifetime);
    try {
        const signedInit = await signTransactionMessageWithSigners(initTransactionMessage);
    ...
    const commitment = await hashMethod.call(bb, [nullifier, secret]);
    ...
    const pushRootMessageWithPayer = setTransactionMessageFeePayerSigner(payer, pushRootMessage);
    const pushRootMessageWithLifetime = setTransactionMessageLifetimeUsingBlockhash(latestBlockhash2, pushRootMessageWithPayer);
    const pushRootTransactionMessage = appendTransactionMessageInstructions([getSetComputeUnitLimitInstruction({ units: 200000 }), pushRootIx, transferIx], pushRootMessageWithLifetime);
    const signedPushRoot = await signTransactionMessageWithSigners(pushRootTransactionMessage);
    ...
    // Calculate nullifier hash - need a new Barretenberg instance
    const bb2 = await Barretenberg.new();
    const FrForHash = await getFrClass(bb2);
    const nullifierFr = FrForHash.fromString(nullifier.toString());
    const hashMethod2 = bb2.poseidon2Hash || bb2.poseidonHash;
    if (!hashMethod2) {
        throw new Error("No poseidon hash method found on Barretenberg");
    }
    const nullifierHash = await hashMethod2.call(bb2, [nullifierFr]);
    // Convert recipient address to field
    const bb3 = await Barretenberg.new();
    const FrClass = await getFrClass(bb3);
    const recipientBytes = Buffer.from(recipient.address.slice(2), "hex");
    const recipientField = FrClass.fromBuffer(recipientBytes);
    const inputs = {
        root: root,
    ...
    const bb4 = await Barretenberg.new();
    const FrClass2 = await getFrClass(bb4);
    const nullifierHashField = FrClass2.fromString(nullifierHash.toString());
    const nullifierHashBytes = new Uint8Array(32);
    // Convert field to bytes (little-endian, 32 bytes)
    const nullifierHashBigInt = BigInt(nullifierHashField.toString());
    for (let i = 0; i < 32; i++) {
        nullifierHashBytes[i] = Number((nullifierHashBigInt >> BigInt(i * 8)) & BigInt(0xff));
    }
    ...
    const recipientFieldBigInt = BigInt(recipientField.toString());
    for (let i = 0; i < 32; i++) {
    ...
    const withdrawMessageWithPayer = setTransactionMessageFeePayerSigner(payer, withdrawMessage);
    const withdrawMessageWithLifetime = setTransactionMessageLifetimeUsingBlockhash(latestBlockhash3, withdrawMessageWithPayer);
    const withdrawTransactionMessage = appendTransactionMessageInstructions([getSetComputeUnitLimitInstruction({ units: 1000000 }), withdrawIx], withdrawMessageWithLifetime);
    const recipientBalanceBefore = await getBalance(ctx, recipient.address);
    const signedWithdraw = await signTransactionMessageWithSigners(withdrawTransactionMessage);
    ...
    const withdrawMessageWithPayer2 = setTransactionMessageFeePayerSigner(payer, withdrawMessage2);
    const withdrawMessageWithLifetime2 = setTransactionMessageLifetimeUsingBlockhash(latestBlockhash4, withdrawMessageWithPayer2);
    const withdrawTransactionMessage2 = appendTransactionMessageInstructions([getSetComputeUnitLimitInstruction({ units: 1000000 }), withdrawIx2], withdrawMessageWithLifetime2);
    try {
        const signedWithdraw2 = await signTransactionMessageWithSigners(withdrawTransactionMessage2);
